...
ride_days = unique(smalldataSet$ServiceDate)
#Creates array of all the different client IDs
clients = unique(smalldataSet$ClientID)
...
write.table(costDF, file = "./main_repo/Access_Analysis_Rproject/Data/Cost_analysisTesting.csv", col.names = T, sep = ",", row.names = F)
    ...
    for (j in 1:length(clients)){
      ...
      if((length(instances) %% 2 == 0) & (length(instances) !=0)){
        for(jj in 1:(length(instances)/2)){
          ...
          write.table(newrow, file = "./main_repo/Access_Analysis_Rproject/Data/Cost_analysisTesting.csv",
...
cost_data <- read.csv("./main_repo/Access_Analysis_Rproject/Data/Cost_analysis.csv", header = T)
