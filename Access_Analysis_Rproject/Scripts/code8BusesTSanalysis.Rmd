...
In the following analyses, we analyze the daily percentage of out-of-service buses. Buses become out-of-service generally after breaking down or if they are unable to arrive at their destinations. Predicting out-of-service buses is important from an operational perspective because KCM and their contractors can better manage their vehicle fleet by preparing adequate standby vehicles each day.
 ...
 * On average, 7.47% of buses are out-of-service each day
 * No deterministic trend over time
 * Some weekly seasonal variation (more on that later)
 * A snowstorm on February 9th 2014 coincided with the highest daily percentage of out-of 
   service buses
...
```{r}
# creating percentage broken buses variable
pctCode8<-(oos_runs/total_runs)*100
code8Buses$pctCode8<-pctCode8
meanCode8<-summary(pctCode8)[4]
meanCode8

# determining max point in data 
maxpctCode8<-code8Buses[which(code8Buses$pctCode8 ==max(code8Buses$pctCode8)),]
maxpctCode8

# max day in for percentage broken buses is due to snowstorm
# http://www.seattleweatherblog.com/snow/winter-wonderland-seattle-sees-biggest-february-snowfall-in-13-years/

# Plot - Percentage of Out-of-Service Buses Plot
...
```{r}
# creating percentage out of service time series
...
all.cv.pctCode8 <- cbind(cv.ar1i1ma2.pctCode8, cv.ar1i1ma1.pctCode8, cv.ar2i1ma2.pctCode8, cv.ar2i1ma1.pctCode8, cv.ar1i0ma2.pctCode8, cv.ar2i0ma1.pctCode8,cv.ar2i0ma2.pctCode8)
avg.cv.pctCode8<-apply(all.cv.pctCode8, 2, mean)
models <- c("AR1I1MA2", "AR2I1MA2","AR1I1MA1", "AR2I1MA1", "AR1MA2", "AR2MA1", "AR2MA2")

# intermodel comparisons
intermodel.comparison.pctCode8<-as.data.frame(cbind(models,round(all.arima.aic.pctCode8,3),round(all.arima.se.pctCode8,3), round(avg.cv.pctCode8,3))) 
names(intermodel.comparison.pctCode8)<-c("models","AIC","SE","MAE") 
intermodel.comparison.pctCode8

col <- c("Red", "Yellow", "Blue", "Green", "Orange", "Black", "Purple")

# Plot - ARIMA Cross Validation
#png(filename = "Plot - ARIMA Cross-Validation.png")
matplot(all.cv.pctCode8, type="l", col=col, lty=1, ylab="Mean Absolute Error", xlab="Periods Forward", main="Cross-validation of ARIMA models", xlim=c(1,13))
text(models, x=rep(12,length(models)), y=all.cv.pctCode8[nrow(all.cv.pctCode8),], col=col)
#dev.off()

# ar1ma2 is best model
# best combination of lowest AIC and MAE
# final model
arima.final.model.pctCode8 <- arima.ar1i0ma2.pctCode8
