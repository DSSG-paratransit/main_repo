...
dataSet <- read.csv("/Users/fineiskid/Desktop/Access_Analysis_Rproject_local/data/UW_Trip_Data_QC.csv")
dataSet$ServiceDate <- as.timeDate(as.character(dataSet$ServiceDate))
ugVec <- numeric(nrows(upperCosts))
for (kk in 1:nrow(upperCosts)){
  ugly <- upperCosts[kk,]
  ugCli <- which(dataSet$ServiceDate == ugly$ServiceDate & dataSet$Run == ugly$Run
                 & dataSet$ClientID== ugly$ClientId & dataSet$LAT == ugly$LatStart
                 & dataSet$LON == ugly$LonStart)
