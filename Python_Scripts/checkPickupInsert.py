...
def checkPickupInsert(URID, busRun):
	subset = busRun.loc[busRun.ETA < URID.pickupWindowEnd,['ETA', 'Lat', 'Lon']]
	for i in subset.index.tolist(): 
		...
		if newETA < URID.pickupWindowEnd:
			if newETA + times[1] < busRun.ETA[i + 1] + 1800:
				newRow = DataFrame({'SchedTime': 'NA', 'ETA' : newETA, 'Lat': URID.latPickup , 'Lon': URID.lonPickup, 'BookingId': URID.BookingId, 'SpaceOn': 'later', 'SpaceOff': 'later', 'Activity': 0,'MobiAidCode': 'later' }) #need to fix those laters
	...
	return(newBusRun) # list of busRun dataframes 
